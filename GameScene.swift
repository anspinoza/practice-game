...
class GameScene: SKScene {
    ...
    // An array that holds all of the current sidewalk bricks
    var bricks = [SKSpriteNode]()
    
    // The size of the sidewalk brick graphics used
    var brickSize = CGSize.zero
    
    // Setting how fast the game is scrolling to the right
    // This may increase as the user progresses through the game
    ...
    func updateBricks(withScrollAmount currentScrollAmount: CGFloat) {
        ...
        for brick in bricks {
            ...
            if newX < -brickSize.width {
                ...
                // Update our farthest-right position trakcer
    ...
    @objc func handleTap(tapGesture: UITapGestureRecognizer) {
        ...
        // Make the skater jump if the player taps while the skater is on the ground
        if skater.isOnGround {
            
            // Set the skater's y-velocity to the skater's initioal jump speed
